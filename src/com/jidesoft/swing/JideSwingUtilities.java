...
public class JideSwingUtilities implements SwingConstants {
	...
	/**
	 * Create a Panel around a component so that component is right in the
	 * middle.
	 * 
	 * @param object
	 *            the component
	 ...
	 *            the component
	 * @param constraint
	 *            the constraint as in BorderLayout. You can use all the
	 *            constraints as in BorderLayout except CENTER.
	 * @return the container which has both the label and the component.
	 */
	public static JPanel createLabeledComponent(JLabel title,
			Component component, Object constraint) {
	...
	public static void centerWindow(Window childToCenter) {
		...
		//
		// int width = (parentWindow.getWidth() - childToCenter.getWidth()) >>
		// 1;
		// int height = (parentWindow.getHeight() - childToCenter.getHeight())
		// >> 1;
		//
		// // according to javadoc of setLocation, it's relevant to parent
		// window. but it's not the case.
	...
	public static void globalCenterWindow(Window childToCenter) {
		...
		// childToCenter.setLocation((screenDim.width - frameDim.width) >> 1,
		// (screenDim.height - frameDim.height) >> 1);
	}
	 ...
	 */
	public static void paintArrow(Graphics g, Color color, int startX,
			int startY, int width, int orientation) {
		Color oldColor = g.getColor();
		g.setColor(color);
		width = width / 2 * 2 + 1; // make sure it's odd
		if (orientation == HORIZONTAL) {
			for (int i = 0; i < (width + 1) / 2; i++) {
				g.drawLine(startX + i, startY + i, startX + width - i - 1,
						startY + i);
			}
		} else if (orientation == VERTICAL) {
			for (int i = 0; i < (width + 1) / 2; i++) {
				g.drawLine(startX + i, startY + i, startX + i, startY + width
						- i - 1);
			}
		}
		g.setColor(oldColor);
	 ...
	 *            horizontal or vertical
	 */
	public static void paintArrow(JComponent c, Graphics g, Color color,
			int startX, int startY, int width, int orientation) {
		if (!c.getComponentOrientation().isLeftToRight()) {
			Color oldColor = g.getColor();
			g.setColor(color);
			width = width / 2 * 2 + 1; // make sure it's odd
			for (int i = 0; i < (width + 1) / 2; i++) {
				g.drawLine(startX + width - i, startY + i, startX + width - i,
						startY + width - i - 1);
			}
			g.setColor(oldColor);
			return;
	...
	public static void paintCross(Graphics g, Color color, int centerX,
		...
		for (int i = 0; i < width; i++) {
			g.drawLine(centerX - size, centerY - size, centerX + size, centerY
					+ size);
			g.drawLine(centerX + size, centerY - size, centerX - size, centerY
					+ size);
			centerX++;
	 ...
	 * @param component
	 *            the component
	 * @return the top level Frame. Null if we didn't find an ancestor which is
	 *         instance of Frame.
	 */
	...
	/**
	 * Synchronizes the two viewports. The view position changes in the master
	 * view, the slave view's view position will change too. Generally speaking,
	 * if you want the two viewports to synchronize vertically, they should have
	 * the same height. If horizontally, the same width.
	 * <p/>
	 * It's OK if you call this method with the same master viewport and slave
	 * viewport duplicate times. It won't cause multiple events fired.
	 * 
	 ...
	 *            the slave viewport
	 * @param orientation
	 *            the orientation. It could be either SwingConstants.HORIZONTAL
	...
	public static void synchronizeView(final JViewport masterViewport,
		...
		if (i >= changeListeners.length) {
			masterViewport
					.addChangeListener(getViewportSynchronizationChangeListener());
		}

		Object property = masterViewport
				.getClientProperty(JideScrollPane.CLIENT_PROPERTY_SLAVE_VIEWPORT);
		...
		masterViewport
				.putClientProperty(
						JideScrollPane.CLIENT_PROPERTY_SLAVE_VIEWPORT,
						slaveViewportMap);

		property = slaveViewport
				.getClientProperty(JideScrollPane.CLIENT_PROPERTY_MASTER_VIEWPORT);
		if (!(property instanceof Map)) {
			property = new HashMap<JViewport, Integer>();
		}
		Map masterViewportMap = (Map) property;
		masterViewportMap.put(masterViewport, orientation);
		slaveViewport.putClientProperty(
				JideScrollPane.CLIENT_PROPERTY_MASTER_VIEWPORT,
				masterViewportMap);
	}
	 ...
	 *            the slave viewport
	 */
	public static void unsynchronizeView(final JViewport masterViewport,
		...
		if (property instanceof Map) {
			...
			if (slaveViewportMap.isEmpty()) {
				...
				masterViewport
						.removeChangeListener(getViewportSynchronizationChangeListener());
			}
			masterViewport.putClientProperty(
					JideScrollPane.CLIENT_PROPERTY_SLAVE_VIEWPORT,
					slaveViewportMap);
		}
	...
	public static int getButtonState(AbstractButton b) {
		...
		if (!model.isEnabled()) {
			...
			if (model.isSelected()) {
				return ThemePainter.STATE_PRESSED; // should be rollover
													// selected
			} else {
	...
	/**
	 * Checks if the two objects equal. If both are null, they are equal. If o1
	 * and o2 both are Comparable, we will use compareTo method to see if it
	 * equals 0. At last, we will use <code>o1.equals(o2)</code> to compare. If
	 * none of the above conditions match, we return false.
	 * 
	 * @param o1
	...
	/**
	 * Checks if the two objects equal. If both are the same instance, they are
	 * equal. If both are null, they are equal. If o1 and o2 both are
	 * Comparable, we will use compareTo method to see if it equals 0. If
	 * considerArrayOrList is true and o1 and o2 are both array, we will compare
	 * each element in the array. At last, we will use
	 * <code>o1.equals(o2)</code> to compare. If none of the above conditions
	 ...
	 * @param o2
	 *            the second object to compare
	 * @param considerArrayOrList
	 *            If true, and if o1 and o2 are both array, we will compare each
	 *            element in the array instead of just compare the two array
	 *            objects.
	 * @return true if the two objects are equal. Otherwise false.
	 */
	public static boolean equals(Object o1, Object o2,
		...
		if (o1 == o2) {
		...
		} else if (considerArrayOrList && o1 instanceof List
				&& o2 instanceof List) {
			...
			if (considerArrayOrList && o1.getClass().isArray()
					&& o2.getClass().isArray()) {
	...
	private static class GetPropertyAction implements
			java.security.PrivilegedAction {
		private String theProp;
		private String defaultVal;

		/**
		 * Constructor that takes the name of the system property whose string
	 ...
	 * is actually one big reason for people to think Swing application ugly. To
	 * address this issue, we changed the code to force to use Tahoma font for
	 * all the fonts in L&F instead of using the system font.
	 * <p/>
	 * However this is a downside to this. Tahoma cannot display Unicode
	 * characters such as Chinese, Japanese and Korean. So if the locale is CJK
	 * ({@link SystemInfo#isCJKLocale()}, we shouldn't use Tahoma. If you are on
	 * JDK 1.5 and above, you shouldn't force to use Tahoma either because JDK
	 * fixed it in 1.5 and above.
	 * <p/>
	 * There are also a few system properties you can set to control if system
	 * font should be used. "swing.useSystemFontSettings" is the one for all
	 * Swing applications. "Application.useSystemFontSettings" is the one for a
	 * particular Swing application.
	 * <p/>
	 ...
	 * locale, it will return true. If "swing.useSystemFontSettings" property us
	 * true, it will return true. If "Application.useSystemFontSettings"
	 * property is true, it will return true. Otherwise, it will return false.
	 * All JIDE L&F considered the returned value and decide if Tahoma font
	 * should be used or not.
	 * <p/>
	 * Last but the least, we also add system property "jide.useSystemfont"
	 * which has the highest priority. If you set it to "true" or "false", this
	 * method will just check that value and return true or false respectively
	 * without looking at any other settings.
	 * 
	...
	public static boolean shouldUseSystemFont() {
		...
		try {
			systemFonts = (String) java.security.AccessController
		...
		boolean useSystemFontSettings = (systemFonts != null && Boolean
				.valueOf(systemFonts));

		if (useSystemFontSettings) {
			Object value = UIDefaultsLookup
					.get("Application.useSystemFontSettings");

			useSystemFontSettings = (value != null || Boolean.TRUE
					.equals(value));
		}

		return "true".equals(SecurityUtils.getProperty("defaultFont", "false"))
	...
	public interface Handler {
		/**
		 * If true, it will call {@link #action(java.awt.Component)} on this
		 * component.
		 * 
		 * @param c
		 *            the component
		...
		/**
		 * The action you want to perform on this component. This method will
		 * only be called if {@link #condition(java.awt.Component)} returns
		 * true.
		 * 
		 * @param c
		 *            the component
		...
		/**
		 * The action you want to perform to any components. If action(c) is
		 * called, this action is after it.
		 * 
		 * @param c
		 *            the component.
		 */
	...
	public interface ConditionHandler extends Handler {
		/**
		 * If this method returns true, the recursive call will stop at the
		 * component and will not call to its children.
		 * 
		 * @param c
		 *            the component
		 * @return true or false.
	...
	/**
	 * A simple handler used by getRecursively. <code><pre>
	 *  if ( condition() ) {
	 *      return action();
	 *  }
	 * </pre></code>. Here is an example to get the first child of the specified
	 * type. <code><pre>
	...
	public interface GetHandler {
		 ...
		 * @param c
		 *            the component
		 * @return the component that will be returned from
		 *         {@link com.jidesoft.swing.JideSwingUtilities#getRecursively(java.awt.Component, com.jidesoft.swing.JideSwingUtilities.GetHandler)}
		 *         .
		 */
	...
	private static void setRecursively0(final Component c, final Handler handler) {
		...
		if (handler instanceof ConditionHandler
				&& ((ConditionHandler) handler).stopCondition(c)) {
			return;
		}
	...
	/**
	 * Gets the first component inside the specified container that has the
	 * specified name.
	 * 
	 * @param c
	 *            the container
	 ...
	 */
	public static Component findFirstComponentByName(final Container c,
			final String name) {
	 ...
	 * @return the component. Null if not found.
	 */
	public static Component findFirstComponentByClass(final Container c,
			final Class<?> clazz) {
		if (clazz != null) {
			return getRecursively(c, new GetHandler() {
				@Override
	 ...
	 */
	public static Component getRecursively(final Component c,
			final GetHandler handler) {
		return getRecursively0(c, handler);
	}

	private static Component getRecursively0(final Component c,
			final GetHandler handler) {
		if (handler.condition(c)) {
	...
	/**
	 * Calls setEnabled method recursively on component. <code>Component</code>
	 * c is usually a <code>Container</code>
	 * 
	 * @param c
	 *            component
	 * @param enabled
	 ...
	 */
	public static void setEnabledRecursively(final Component c,
	...
	/**
	 * Calls putClientProperty method recursively on component and its child
	 * components as long as it is JComponent.
	 * 
	 * @param c
	 *            component
	 * @param clientProperty
	 *            the client property name
	 * @param value
	 *            the value for the client property
	 */
	public static void putClientPropertyRecursively(final Component c,
			final String clientProperty, final Object value) {
		setRecursively(c, new Handler() {
			public boolean condition(Component c) {
	 ...
	 */
	public static void setRequestFocusEnabledRecursively(final Component c,
			final boolean enabled) {
		setRecursively(c, new Handler() {
			public boolean condition(Component c) {
				return true;
	...
	/**
	 * setOpaqueRecursively method will make all child components opaque true or
	 * false. But if you call
	 * jcomponent.putClientProperty(SET_OPAQUE_RECURSIVELY_EXCLUDED,
	 * Boolean.TRUE), we will not touch this particular component when
	 * setOpaqueRecursively.
	 */
	public static final String SET_OPAQUE_RECURSIVELY_EXCLUDED = "setOpaqueRecursively.excluded";

	/**
	 * Calls setOpaque method recursively on each component except for JButton,
	 * JComboBox and JTextComponent. <code>Component</code> c is usually a
	 * <code>Container</code>. If you would like certain child component not
	 * affected by this call, you can call
	 * jcomponent.putClientProperty(SET_OPAQUE_RECURSIVELY_EXCLUDED,
	 * Boolean.TRUE) before calling this method.
	 * 
	 * @param c
	 *            component
	 * @param opaque
	 ...
	 */
	public static void setOpaqueRecursively(final Component c,
			final boolean opaque) {
		setRecursively(c, new Handler() {
			public boolean condition(Component c) {
				return !(c instanceof JComboBox || c instanceof JButton
						|| c instanceof JTextComponent
						|| c instanceof ListCellRenderer
						|| c instanceof TreeCellRenderer
						|| c instanceof TableCellRenderer || c instanceof CellEditor);
			}
			...
			public void action(Component c) {
				if (c instanceof JComponent) {
					...
					if (Boolean.TRUE.equals(jc
							.getClientProperty(SET_OPAQUE_RECURSIVELY_EXCLUDED))) {
						return;
					}

					Object clientProperty = jc
							.getClientProperty(OPAQUE_LISTENER);
					if (clientProperty != null) {
						jc.removePropertyChangeListener("opaque",
								(PropertyChangeListener) clientProperty);
					...
					if (jc.getClientProperty(OPAQUE_LISTENER) == null) {
						if (opaque) {
								...
								_setOpaqueTrueListener = new PropertyChangeListener() {
									public void propertyChange(
										...
										if (evt.getSource() instanceof JComponent) {
											Component component = ((Component) evt
													.getSource());
											component
							...
							jc.addPropertyChangeListener("opaque",
									_setOpaqueTrueListener);
							jc.putClientProperty(OPAQUE_LISTENER,
									_setOpaqueTrueListener);
						} else {
	...
	public static Dimension getPreferredButtonSize(AbstractButton b,
			int textIconGap, boolean isHorizontal) {
	 ...
	 *            the icon
	 * @param isHorizontal
	 *            the flag indicating horizontal or vertical
	...
	public static String layoutCompoundLabel(JComponent c, FontMetrics fm,
			String text, Icon icon, boolean isHorizontal,
		...
		return layoutCompoundLabelImpl(c, fm, text, icon, isHorizontal,
				verticalAlignment, hAlign, verticalTextPosition, hTextPos,
	...
	public static String layoutCompoundLabel(FontMetrics fm, String text,
			Icon icon, boolean isHorizontal, int verticalAlignment,
		...
		return layoutCompoundLabelImpl(null, fm, text, icon, isHorizontal,
	...
	private static String layoutCompoundLabelImpl(JComponent c, FontMetrics fm,
		 ...
		 */
		if (isHorizontal)
			return layoutCompoundLabelImplHorizontal(c, fm, text, icon,
					verticalAlignment, horizontalAlignment,
					verticalTextPosition, horizontalTextPosition, viewR, iconR,
					textR, textIconGap);
		else
			return layoutCompoundLabelImplVertical(c, fm, text, icon,
					verticalAlignment, horizontalAlignment,
					verticalTextPosition, horizontalTextPosition, viewR, iconR,
					textR, textIconGap);
	...
	private static String getMaxLengthWord(String text) {
		if (text.indexOf(' ') == -1) {
			...
			return minPos >= mid ? text.substring(0, minPos) : text
					.substring(minPos + 1);
		}
	}

	private static String layoutCompoundLabelImplHorizontal(JComponent c,
			FontMetrics fm, String text, Icon icon, int verticalAlignment,
			int horizontalAlignment, int verticalTextPosition,
			int horizontalTextPosition, Rectangle viewR, Rectangle iconR,
			Rectangle textR, int textIconGap) {
		/*
			...
			if (v != null) {
				...
				if (false) { // TODO: debug switch
					...
					if (verticalTextPosition == BOTTOM
							&& horizontalTextPosition == CENTER) { // in this
																	// case, we
																	// will wrap
																	// the text
																	// into two
					...
					if (wrapText) {
						textR.width = SwingUtilities.computeStringWidth(fm,
								getMaxLengthWord(text));
						textR.height = fm.getHeight() + fm.getAscent() + 2; // gap
																			// between
																			// the
																			// two
																			// lines
						...
						textR.width = SwingUtilities.computeStringWidth(fm,
								text) + 1; // add an extra pixel at the end of
											// the text
						textR.height = fm.getHeight();
					}
				} else {
					textR.width = SwingUtilities.computeStringWidth(fm, text); // add
																				// an
																				// extra
																				// pixel
																				// at
		...
		if (verticalAlignment == TOP) {
			...
			dy = (viewR.y + (viewR.height >> 1))
					- (labelR_y + (labelR_height >> 1));
	...
	private static String layoutCompoundLabelImplVertical(JComponent c,
		...
		if (!textIsEmpty) {
			...
			if (textR.height > availTextHeight) {
				if (v != null) {
					...
					String clipString = "...";
					int totalHeight = SwingUtilities.computeStringWidth(fm,
							clipString);
					int nChars;
					for (nChars = 0; nChars < text.length(); nChars++) {
		...
		int labelR_width = Math.max(iconR.y + iconR.height, textR.y
				+ textR.height)
				- labelR_x;
		int labelR_y = Math.min(iconR.x, textR.x);
		int labelR_height = Math.max(iconR.x + iconR.width, textR.x
				+ textR.width)
				- labelR_y;
	...
	public static int getOrientationOf(Component component) {
		if (component instanceof Alignable) {
			...
			Integer value = (Integer) ((JComponent) component)
					.getClientProperty(Alignable.PROPERTY_ORIENTATION);
			if (value != null)
				return value;
		}
		return HORIZONTAL;
		...
		if (orientation != old) {
			if (component instanceof Alignable) {
				...
				((JComponent) component).putClientProperty(
						Alignable.PROPERTY_ORIENTATION, orientation);
			}
		}
	}
	...
	/**
	 * Disables the double buffered flag of the component and its children. The
	 * return map contains the components that were double buffered. After this
	 * call, you can then restore the double buffered flag using
	 * {@link #restoreDoubleBuffered(java.awt.Component, java.util.Map)} using
	 ...
	 */
	public static Map<Component, Boolean> disableDoubleBuffered(
			final Component c) {
		final Map<Component, Boolean> map = new HashMap<Component, Boolean>();
		if (c instanceof JComponent) {
			JideSwingUtilities.setRecursively(c,
	...
	/**
	 * Enables the double buffered flag of the component and its children. The
	 * return map contains the components that weren't double buffered. After
	 * this call, you can then restore the double buffered flag using
	 * {@link #restoreDoubleBuffered(java.awt.Component, java.util.Map)} using
	 * the map that is returned from this method.
	 * 
	 * @param c
	 *            the parent container.
	 * @return the map that contains all components that weren't double
	 *         buffered.
	 */
	...
	/**
	 * Restores the double buffered flag of the component and its children. Only
	 * components that are in the map will be changed.
	 * 
	 * @param c
	 *            the parent container.
	 * @param map
	 *            a map maps from component to a boolean. If the boolean is
	 *            true, it means the component was double buffered bore.
	 *            Otherwise, not double buffered.
	 */
	public static void restoreDoubleBuffered(final Component c,
			final Map<Component, Boolean> map) {
		JideSwingUtilities.setRecursively(c, new JideSwingUtilities.Handler() {
			...
			public void action(Component c) {
				...
				if (value != null) {
					((JComponent) c).setDoubleBuffered(Boolean.TRUE
							.equals(value));
				}
			}
	...
	public static void paintBackground(Graphics2D g2d, Rectangle rect,
			Color border, Paint paint) {
	 ...
	 * @param c
	 *            JComponent to test.
	 * @return Whether or not text should be drawn anti-aliased for the
	 *         specified component.
	 */
	private static boolean drawTextAntialiased(Component c) {
		if (!AA_TEXT_DEFINED) {
			if (c != null) {
				...
				if (c instanceof JComponent) {
					Boolean aaProperty = (Boolean) ((JComponent) c)
							.getClientProperty(AA_TEXT_PROPERTY_KEY);
	...
	static {
		if (SystemInfo.isJdk6Above()) {
			...
			renderingHints = (Map) (tk
					.getDesktopProperty("awt.font.desktophints"));
			tk.addPropertyChangeListener("awt.font.desktophints",
					new PropertyChangeListener() {
						public void propertyChange(PropertyChangeEvent evt) {
							if (evt.getNewValue() instanceof RenderingHints) {
								renderingHints = (RenderingHints) evt
										.getNewValue();
							}
						}
					});
	 ...
	 */
	private static Map getRenderingHints(Graphics2D g2d, Map hintsToSave,
			Map savedHints) {
		if (savedHints == null) {
	...
	public static void drawString(JComponent c, Graphics g, String text, int x,
			int y) {
		if (SystemInfo.isJdk6Above()) {
	 ...
	 * <p/>
	 * Under JDK1.4 and JDK5, this method will use a system property
	 * "swing.aatext" to determine if anti-alias is used. Under JDK6, we will
	 * read the system setting. For example, on Windows XP, there is a check box
	 * to turn on clear type anti-alias. We will use the same settings.
	 * 
	 * @param c
	 *            the component
	 * @param g
	 *            the Graphics instance
	 * @return the old hints. You will need this value as the third parameter in
	 *         {@link #restoreAntialiasing(java.awt.Component, java.awt.Graphics, Object)}
	 *         .
	 */
	 ...
	 *            {@link #setupAntialiasing(java.awt.Component, java.awt.Graphics)}
	 *            .
	 */
	public static void restoreAntialiasing(Component c, Graphics g,
			Object oldHints) {
		Graphics2D g2d = (Graphics2D) g;
		if (SystemInfo.isJdk6Above()) {
	 ...
	 * @param g
	 * @return the old hints. You will need this value as the third parameter in
	 *         {@link #restoreShapeAntialiasing(java.awt.Graphics, Object)}.
	 */
	 ...
	 * @param oldHints
	 *            the value returned from
	 *            {@link #setupShapeAntialiasing(java.awt.Graphics)}.
	 */
	...
	public static void drawGrip(Graphics g, Rectangle rectangle, int maxLength,
			int maxThickness) {
		drawGrip(g, rectangle, maxLength, maxThickness, true);
	}

	public static void drawGrip(Graphics g, Rectangle rectangle, int maxLength,
			int maxThickness, boolean isSelected) {
		if (rectangle.width > rectangle.height) {
			...
			int startY = rectangle.y
					+ ((rectangle.height - (maxThickness * 3)) >> 1);
			for (int i = 0; i < maxThickness; i++) {
				for (int j = 0; j < count; j++) {
					if (isSelected) {
						g.setColor(UIDefaultsLookup
								.getColor("controlLtHighlight"));
						g.drawLine(startX + j * 3, startY + i * 3, startX + j
								* 3, startY + i * 3);
					}
					g.setColor(UIDefaultsLookup.getColor("controlShadow"));
					g.drawLine(startX + j * 3 + 1, startY + i * 3 + 1, startX
							+ j * 3 + 1, startY + i * 3 + 1);
				}
			}
		} else {
			...
			int startX = rectangle.x
					+ ((rectangle.width - (maxThickness * 3)) >> 1);
			...
			for (int i = 0; i < maxThickness; i++) {
				for (int j = 0; j < count; j++) {
					if (isSelected) {
						g.setColor(UIDefaultsLookup
								.getColor("controlLtHighlight"));
						g.drawLine(startX + i * 3, startY + j * 3, startX + i
								* 3, startY + j * 3);
					}
					g.setColor(UIDefaultsLookup.getColor("controlShadow"));
					g.drawLine(startX + i * 3 + 1, startY + j * 3 + 1, startX
							+ i * 3 + 1, startY + j * 3 + 1);
				}
			}
		}
	...
	public static void fillGradient(Graphics g, Rectangle rect, int orientation) {
		...
		if (orientation == SwingConstants.HORIZONTAL) {
			// paint upper gradient
			fillGradient(g2d, new Rectangle(rect.x, rect.y, rect.width,
					rect.height >> 1), col2, col1, true);

			// paint lower gradient
			fillGradient(g2d, new Rectangle(rect.x,
					rect.y + (rect.height >> 1), rect.width, rect.height >> 1),
					col3, col4, true);
		} else {
			// paint left gradient
			fillGradient(g2d, new Rectangle(rect.x, rect.y, rect.width >> 1,
					rect.height), col2, col1, false);

			// paint right gradient
			fillGradient(g2d, new Rectangle(rect.x + (rect.width >> 1), rect.y,
					rect.width >> 1, rect.height), col3, col4, false);
		}
	}

	public static void fillSingleGradient(Graphics g, Rectangle rect,
			int orientation) {
	...
	/**
	 * Gets the RadialGradientPaint. RadialGradientPaint is added after JDK6. If
	 * you are running JDK5 or before, you can include batik-awt-util.jar which
	 * also has a RadialGradientPaint class. This method will use reflection to
	 * determine if the RadialGradientPaint class is in the class path and use
	 * the one it can find.
	 */
	public static Paint getRadialGradientPaint(Point2D point, float radius,
			float[] fractions, Color[] colors) {
		Class<?> radialGradientPaintClass = null;
		try {
			if (SystemInfo.isJdk6Above()) {
				radialGradientPaintClass = Class
						.forName("java.awt.RadialGradientPaint");
			} else {
				radialGradientPaintClass = Class
						.forName("org.apache.batik.ext.awt.RadialGradientPaint");
			}
		...
		if (radialGradientPaintClass != null) {
			try {
				if (_radialGradientPaintConstructor2 == null) {
					_radialGradientPaintConstructor2 = radialGradientPaintClass
							.getConstructor(new Class[] { Point2D.class,
									float.class, float[].class, Color[].class });
				}
				final Object radialGradientPaint = _radialGradientPaintConstructor2
						.newInstance(point, radius, fractions, colors);
		...
		System.err
				.println("Warning - radial gradients are only supported in Java 6 and higher or use batik-aw-util.jar, using a plain color instead"); //$NON-NLS-1$
		return colors[0];
	}
	 ...
	 * the one it can find.
	 */
	public static Paint getRadialGradientPaint(float cx, float cy,
			float radius, float[] fractions, Color[] colors) {
		...
		if (_radialGradientPaintClass != null) {
			try {
				if (_radialGradientPaintConstructor1 == null) {
					_radialGradientPaintConstructor1 = _radialGradientPaintClass
							.getConstructor(new Class[] { float.class,float.class, float.class, float[].class,Color[].class });
				}
				final Object radialGradientPaint = _radialGradientPaintConstructor1
						.newInstance(cx, cy, radius, fractions, colors);
				return (Paint) radialGradientPaint;
			} catch (NoSuchMethodException e) {
				// ignore
			} catch (InstantiationException e) {
	...
	/**
	 * Gets the LinearGradientPaint. LinearGradientPaint is added after JDK6. If
	 * you are running JDK5 or before, you can include batik-awt-util.jar which
	 * also has a LinearGradientPaint class. This method will use reflection to
	 * determine if the LinearGradientPaint class is in the class path and use
	 * the one it can find.
	 */
	public static Paint getLinearGradientPaint(float startX, float startY,
		...
		if (_linearGradientPaintClass != null) {
			try {
				...
				final Object linearGradientPaint = _linearGradientPaintConstructor1
						.newInstance(startX, startY, endX, endY, fractions,
								colors);
	 ...
	 * @return Is the current focusOwner a descendant of the specified
	...
	public static void fillNormalGradient(Graphics2D g2d, Shape s,Color startColor, Color endColor, boolean isVertical) {
		...
		GradientPaint paint;
		if (isVertical) {
			paint = new GradientPaint(rect.x, rect.y, startColor, rect.x,rect.height + rect.y, endColor, true); // turn cyclic to
															// true will be
															// faster
		} else {
			paint = new GradientPaint(rect.x, rect.y, startColor, rect.width + rect.x, rect.y, endColor, true); // turn cyclic to true
														// will be faster
		}
	...
	/**
	 * Fills a gradient using the startColor and endColor specified. This is a
	 * fast version of fill gradient which will not only leverage hardware
	 * acceleration, but also cache GradientPaint and reuse it.
	 * <p/>
	 * We also leave an option to use the normal GradientPaint to paint the
	 * gradient. To do so, just set a system property "normalGradientPaint" to
	 * "false".
	 * 
	 * @param g2d
	 * @param s
	 ...
	 * @param isVertical
	 */
	public static void fillGradient(Graphics2D g2d, Shape s, Color startColor,
	...
	public static Window getTopModalDialog(Window window) {
		Window[] ws = window.getOwnedWindows();
		for (Window w1 : ws) {
			if (w1.isVisible() && w1 instanceof Dialog
					&& ((Dialog) w1).isModal()) {
			...
			public void propertyChange(PropertyChangeEvent evt) {
				if (useBorders) {
					...
					if (newValue instanceof JComponent) {
						...
						if (!(oldBorder instanceof TraceDebugBorder))
							((JComponent) newValue)
									.setBorder(new TraceDebugBorder(oldBorder));
					}
				}

				String oldName = evt.getOldValue() == null ? "null" : evt
						.getOldValue().getClass().getName();
				if (evt.getOldValue() instanceof Component
						&& ((Component) evt.getOldValue()).getName() != null)
					oldName = oldName + "'"
							+ ((Component) evt.getOldValue()).getName() + "'";
				String newName = evt.getNewValue() == null ? "null" : evt
						.getNewValue().getClass().getName();
				if (evt.getNewValue() instanceof Component
						&& ((Component) evt.getNewValue()).getName() != null)
					newName = newName + "'"
							+ ((Component) evt.getNewValue()).getName() + "'";
	...
	public static void runGCAndPrintFreeMemory() {
		...
		memUsed = memFormatter.format((Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / 1024);
	 ...
	 * For internal usage only.
	 */
	public static JPanel createTableModelModifier(
	...
	public static boolean passesFocusabilityTest(Component comp) {
		return ((comp != null) && comp.isEnabled() && comp.isDisplayable()
				&& comp.isVisible() && comp.isFocusable() && comp.isShowing());
	}

	/**
	 * Ignore the exception. This method does nothing. However it's a good
	 * practice to use this method so that we can easily find out the place that
	 * ignoring exception. In development phase, we can log a message in this
	 * method so that we can verify if it makes sense to ignore.
	 * 
	 * @param e
	 */
	...
	/**
	 * Throws the exception. If the exception is RuntimeException, just throw
	 * it. Otherwise, wrap it in RuntimeException and throw it.
	 * 
	 * @param e
	 */
	 ...
	 * has a nested exception as target exception. If the target exception is a
	 * RuntimeException or Error, we will throw it. Otherwise, we will wrap it
	...
	/**
	 * Gets the first occurrence of the component with specified type in the
	 * container. It used deep-first searching to find it.
	 * 
	 * @param c
	 * @param container
	 * @return the first occurrence of the component with specified type in the
	 *         container. Null if nothing is found.
	 */
	...
	public static Object getMenuFont(Toolkit toolkit, UIDefaults table) {
		...
		if (JideSwingUtilities.shouldUseSystemFont()) {
			if (defaultFontSize == -1/* || SystemInfo.isCJKLocale() */) {
				...
				menuFont = new WindowsDesktopProperty("win.menu.font",
						table.getFont("ToolBar.font"), toolkit, defaultFontSize);
			}
		} else {
			Font font = table.getFont("ToolBar.font");
			if (font == null) {
				menuFont = SecurityUtils.createFontUIResource("Tahoma",
						Font.PLAIN,
						defaultFontSize != -1f ? (int) defaultFontSize : 11);
			} else {
				menuFont = SecurityUtils.createFontUIResource(
						font.getFontName(),
						Font.PLAIN,
						defaultFontSize != -1f ? (int) defaultFontSize : font
								.getSize());
			}
		}
	...
	public static Object getBoldFont(Toolkit toolkit, UIDefaults table) {
		if (SystemInfo.isCJKLocale()) {
			...
			if (JideSwingUtilities.shouldUseSystemFont()) {
				Font font = table.getFont("Label.font");
				if (font == null) {
					font = new Font("Tahoma", Font.PLAIN, 12); // use default
																// font
				}
	...
	public static void drawShadow(Graphics g, Component c, int x, int y, int w,
			int h) {
		if (w <= 0 || h <= 0) {
			return;
		}
	...
	// public static void drawTileImage(Graphics g, ImageIcon img, Rectangle
	// rect, Insets ins, boolean horizontal) {
	// int left = ins.left;
	// int right = ins.right;
	// int top = ins.top;
	...
	/**
	 * Draws a border based on an image. The image can be divided into nine
	 * different areas. Each area size is determined by the insets.
	 */
	 ...
	 */
	public static boolean isKeyListenerRegistered(Component component,
			KeyListener l) {
	 ...
	 * @param index
	 */
	public static void insertKeyListener(Component component, KeyListener l,
			int index) {
		KeyListener[] listeners = component.getKeyListeners();
		for (KeyListener listener : listeners) {
	...
	/**
	 * Inserts the table model listener at the particular index in the
	 * listeners' chain. The listeners are fired in reverse order. So the
	 * listener at index 0 will be fired at last.
	 * 
	 * @param model
	 *            the AbstractTableModel
	 * @param l
	 *            the TableModelListener to be inserted
	 * @param index
	 *            the index.
	 */
	...
	/**
	 * Checks if the property change listener is already registered on the
	 * component.
	 * 
	 * @param component
	 *            the component
	 ...
	 * @return true if already registered. Otherwise false.
	 */
	public static boolean isPropertyChangeListenerRegistered(
			Component component, PropertyChangeListener l) {
		PropertyChangeListener[] listeners = component
	 ...
	 */
	public static boolean isPropertyChangeListenerRegistered(
			Component component, String propertyName, PropertyChangeListener l) {
	 ...
	 * @return true if already registered. Otherwise false.
	 */
	public static boolean isMouseListenerRegistered(Component component,
			MouseListener l) {
	 ...
	 * @param index
	 */
	public static void insertMouseListener(Component component,
			MouseListener l, int index) {
		MouseListener[] listeners = component.getMouseListeners();
		for (MouseListener listener : listeners) {
	...
	/**
	 * Checks if the mouse motion listener is already registered on the
	 * component.
	 * 
	 * @param component
	 *            the component
	...
	/**
	 * Inserts the mouse motion listener at the particular index in the
	 * listeners' chain.
	 * 
	 * @param component
	 * @param l
	 * @param index
	 */
	public static void insertMouseMotionListener(Component component,
			MouseMotionListener l, int index) {
		MouseMotionListener[] listeners = component.getMouseMotionListeners();
		for (MouseMotionListener listener : listeners) {
	...
	/**
	 * Checks if the listener is always registered to the EventListenerList to
	 * avoid duplicated registration of the same listener
	 * 
	 * @param list
	 *            the EventListenerList to register the listener.
	 * @param t
	 ...
	 * @return true if already registered. Otherwise false.
	 */
	public static boolean isListenerRegistered(EventListenerList list, Class t,
			EventListener l) {
	...
	/**
	 * Checks if the listener is always registered to the Component to avoid
	 * duplicated registration of the same listener
	 * 
	 * @param component
	 *            the component that you want to register the listener.
	 * @param t
	 *            the type of the EventListener.
	 * @param l
	 *            the listener.
	 * @return true if already registered. Otherwise false.
	 */
	public static boolean isListenerRegistered(Component component, Class t,
			EventListener l) {
		Object[] objects = component.getListeners(t);
		return isListenerRegistered(objects, t, l);
	}

	private static boolean isListenerRegistered(Object[] objects, Class t,
			EventListener l) {
	...
	/**
	 * Get the index of the component in the container. It will return -1 if c's
	...
	public static Vector convertDefaultComboBoxModelToVector(
	...
	/**
	 * To make sure the row is visible. If the table's horizontal scroll bar is
	 * visible, the method will not change the horizontal scroll bar's position.
	 * 
	 * @param table
	 * @param row
	 */
	...
	public static void retargetMouseEvent(int id, MouseEvent e, Component target) {
		...
		Point p = SwingUtilities.convertPoint((Component) e.getSource(),
				e.getX(), e.getY(), target);
		MouseEvent retargeted = new MouseEvent(target, id, e.getWhen(),
				e.getModifiersEx() | e.getModifiers(), p.x, p.y,
	...
	/**
	 * Checks if the font specified by the font name is fixed width font. Fixed
	 * width font means all chars have the exact same width.
	 * 
	 * @param fontName
	 *            the font name
	 * @param component
	...
	/**
	 * Sets the locale recursively on the component and all its child components
	 * if any.
	 * 
	 * @param c
	 *            the component
	 ...
	 *            the new locales.
	 */
	public static void setLocaleRecursively(final Component c,
			final Locale locale) {
		JideSwingUtilities.setRecursively(c, new JideSwingUtilities.Handler() {
			public boolean condition(Component c) {
				return true;
	...
	/**
	 * Sets the bounds. If the container orientation is from right to left, this
	 * method will adjust the x to the opposite.
	 * 
	 * @param container
	 *            the container. It is usually the parent of the component.
	 * @param component
	 *            the component to set bounds
	 * @param bounds
	 *            the bounds.
	 */
	public static void setBounds(Container container, Component component,
			Rectangle bounds) {
	 ...
	 *            the y of the bounds
	 * @param width
	 *            the the height of the bounds. of the bounds.
	 * @param height
	...
	/**
	 * Invalidate and doLayout on the component and all its child components if
	 * any.
	 * 
	 * @param c
	 *            the component
	...
	/**
	 * Registers all actions registered on the source component and registered
	 * them on the target component at the specified condition.
	 * 
	 ...
	 *            the keystrokes
	 * @param condition
	 *            the condition which will be used in
	 *            {@link javax.swing.JComponent#registerKeyboardAction(java.awt.event.ActionListener, javax.swing.KeyStroke, int)}
	 *            as the last parameter.
	 */
	public static void synchronizeKeyboardActions(JComponent sourceComponent,
			JComponent targetComponent, KeyStroke[] keyStrokes, int condition) {
	 ...
	 */
	public static JComponent getFirstJComponent(
			RootPaneContainer rootPaneContainer) {
		return (JComponent) getRecursively(rootPaneContainer.getContentPane(),
				new GetHandler() {
					public boolean condition(Component c) {
						return c instanceof JComponent;
					}
	...
	/**
	 * This method can be used to fix two JDK bugs. One is to fix the row height
	 * is wrong when the first element in the model is null or empty string. The
	 * second bug is only on JDK1.4.2 where the vertical scroll bar is shown
	 * even all rows are visible. To use it, you just need to override
	 ...
	 * @param list
	 *            the JList
	 * @param defaultViewportSize
	 *            the default viewport size from
	 *            JList#getPreferredScrollableViewportSize().
	 * @return the adjusted size.
	 */
	public static Dimension adjustPreferredScrollableViewportSize(JList list,
			Dimension defaultViewportSize) {
		// workaround the bug that the list is tiny when the first element is
		// empty
		...
		if (SystemInfo.isJdk15Above()) {
			...
			// in JDK1.4.2, the vertical scroll bar is shown because of the
			// wrong size is calculated.
	...
	/**
	 * The semantics in AWT of hiding a component, removing a component, and
	 * reparenting a component are inconsistent with respect to focus. By
	 * calling this function before any of the operations above focus is
	 * guaranteed a consistent degregation.
	 * 
	 * @param component
	 */
	 ...
	 *            the component
	 * @param defaultHeight
	 *            the default height if the font on the specified component is
	 *            null
	 * @return the line height for the font for the component (or the passed in
	 *         the default value if the font on the specified component is null)
	 */
	...
	/**
	 * Removes extra separators, if any. This can be used when you remove some
	 * menu items and leave extra separators on the UI.
	 * 
	 * @param popup
	 *            the popup menu.
	 */
	public static void removeExtraSeparators(JPopupMenu popup) {
		...
		for (int i = 0; i < components.length; i++) {
			...
			if (component instanceof JSeparator) {
				if (i == 0 || i == components.length - 1) { // if the separator
															// is the first one
															// or the last one,
															// remove it because
															// the separator is
	...
	/**
	 * Sets the text component transparent. It will call setOpaque(false) and
	 * also set client property for certain L&Fs in case the L&F doesn't respect
	 * the opaque flag.
	 * 
	 * @param component
	 *            the text component to be set to transparent.
	 * @deprecated replaced by
	 *             {@link #setComponentTransparent(javax.swing.JComponent)}.
	 */
	@Deprecated
	...
	public static void setComponentTransparent(JComponent component) {
		...
		// add this for Nimbus to disable all the painting of a component in
		// Nimbus
	 ...
	 * @return the index of the given key if it exists in the list, otherwise -1
	 *         times the index value at the insertion point that would be used
	 *         if the key were added to the list.
	 */
	 ...
	 * @param key
	 *            the key to search for
	 * @return the index of the given key if it exists in the array, otherwise
	 *         -1 times the index value at the insertion point that would be
	 *         used if the key were added to the array.
	 */
	 ...
	 *            the key to search for
	 * @param start
	 *            the start index to search inclusive
	 * @param end
	...
	private static class viewportSynchronizationChangeListener implements
			ChangeListener {
		public void stateChanged(ChangeEvent e) {
			...
			do {
				...
				for (JViewport slaveViewport : allViewportToSync.keySet()) {
					Object slaveProperty = slaveViewport
							.getClientProperty(JideScrollPane.CLIENT_PROPERTY_SLAVE_VIEWPORT);
					...
					for (JViewport viewport : viewportMap.keySet()) {
						if (viewport != masterViewport
								&& !allViewportToSync.containsKey(viewport)
								&& viewportMap.get(viewport) == orientation) {
							viewportToAdd.put(viewport,
									viewportMap.get(viewport));
						}
					}
				}
	...
	/**
	 * Sets the Window opaque using AWTUtilities.setWindowOpaque on JDK6u10 and
	 * later.
	 * 
	 * @param window
	 *            the Window
	...
	/**
	 * Sets the Window opacity using AWTUtilities.setWindowOpacity on JDK6u10
	 * and later.
	 * 
	 * @param window
	 *            the Window
	...
	/**
	 * Sets the Window shape using AWTUtilities.setWindowOpacity on JDK6u10 and
	 * later.
	 * 
	 * @param window
	 *            the Window
	...
	public static String getOKString(Locale locale) {
		String text = UIDefaultsLookup.getString("OptionPane.okButtonText",
				locale);
		if (text == null || text.length() <= 0) {
			text = UIDefaultsLookup.getString("ColorChooser.okText");
			if (text == null || text.length() <= 0) {
				text = ButtonResources.getResourceBundle(locale).getString(
						"Button.ok");
			}
		}
		return text;
	...
	public static String getCancelString(Locale locale) {
		String text = UIDefaultsLookup.getString("OptionPane.cancelButtonText",
				locale);
		if (text == null || text.length() <= 0) {
	...
	public static String getYesString(Locale locale) {
		String text = UIDefaultsLookup.getString("OptionPane.yesButtonText",
				locale);
		if (text == null || text.length() <= 0) {
			text = ButtonResources.getResourceBundle(locale).getString(
					"Button.yes");
	...
	public static String getNoString(Locale locale) {
		String text = UIDefaultsLookup.getString("OptionPane.noButtonText",
				locale);
		if (text == null || text.length() <= 0) {
			text = ButtonResources.getResourceBundle(locale).getString(
					"Button.no");
