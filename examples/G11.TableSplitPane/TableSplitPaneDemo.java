...
public class TableSplitPaneDemo extends AbstractDemo {
    ...
    private TableSplitPane createTablePane() {
        ...
        TableSplitPane pane = new TableSplitPane(new FilterableTreeTableModel(tableModel)) {
            @Override
            protected TableScrollPane createTableScrollPane(MultiTableModel tableModel, MultiTableModel footerTableModel, int tableIndex, boolean sortable) {
                if (tableIndex == 0) {
                    TableScrollPane scrollPane = new TableScrollPane(tableModel, footerTableModel, sortable, tableIndex, false) {
                        @Override
                ...
                return super.createTableScrollPane(tableModel, footerTableModel, tableIndex, sortable);
            }
        ...
        AutoFilterTableHeader header = new AutoFilterTableHeader(panes[0].getRowHeaderTable());
        header.setAutoFilterEnabled(true);
        panes[0].getRowHeaderTable().setTableHeader(header);
        ...
        NestedTableHeader nesetedHeader = (NestedTableHeader) table.getTableHeader();
        nesetedHeader.addColumnGroup(first);
        nesetedHeader.addColumnGroup(second);
    ...
    private static class Task extends DefaultExpandableRow {
        ...
        public Object getValueAt(int columnIndex) {
            if (columnIndex >= 6 && columnIndex < 14) {
                return resourceUsed[columnIndex - 6];
            }
            switch (columnIndex) {
                case 0:
                    return name;
        ...
        public void setValueAt(Object value, int columnIndex) {
            if (columnIndex >= 6 && columnIndex < 14 && value instanceof Boolean) {
                resourceUsed[columnIndex - 6] = (Boolean) value;
                return;
            }
            switch (columnIndex) {
                case 0:
